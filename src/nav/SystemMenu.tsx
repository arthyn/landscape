...
export const SystemMenu = ({ open, setOpen, className, showOverlay = false }: SystemMenuProps) => {
  ...
  return (
    <>
      <DropdownMenu.Root open={open} onOpenChange={(open) => setOpen(open)}>
        ...
        <DropdownMenu.Content onCloseAutoFocus={(e) => e.preventDefault() } sideOffset={12} className="dropdown min-w-64 p-6 font-semibold text-gray-500 bg-white">
          <DropdownMenu.Group className="space-y-6">
            ...
            <DropdownMenu.Item as={Link} to="/leap/help-and-support" className="flex items-center space-x-2 default-ring ring-offset-2 rounded" onSelect={(e) => { e.stopPropagation(); e.preventDefault(); setTimeout(() => setOpen(false), 0) }}>
              <span className="w-5 h-5 bg-gray-100 rounded-full"></span>
      ...
      {showOverlay && open && 
        <div className="fixed z-30 right-0 bottom-0 w-screen h-screen bg-black opacity-30"></div>
      }
    </>
  )
}